...
\begin{itemdescr}
\pnum
Let \exposid{as-variant} denote the exposition-only function templates
...
For each $0 \leq i < n$, let $\tcode{FV}_i$ denote the expression
\tcode{std::forward<$\tcode{Variants}_i$>($\tcode{vars}_i$)} and
let $\tcode{V}_i$ denote the type
\tcode{decltype(\exposid{as-variant}(FV$_i$))}.
