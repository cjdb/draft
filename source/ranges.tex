...
\pnum
The member \grammarterm{typedef-name} \tcode{iterator_category} is defined
if and only if \tcode{Base} models \tcode{forward_range}.
In that case,
...
\tcode{Base} models \libconcept{forward_range}, and
\tcode{range_reference_t<Base>} models \libconcept{forward_range}.
In that case,
...
\begin{itemize}
\item
If \tcode{get<N>(*current_)} is an rvalue,
\tcode{iterator_category} denotes \tcode{input_iterator_tag}.
\item
Otherwise, if \tcode{C} models \tcode{derived_from<random_access_iterator_tag>},
\tcode{iterator_category} denotes \tcode{random_access_iterator_tag}.
\item
